...
class bluetooth(modules.Module):
    ...
    menu = {'7': {
        ...
        'listTyp': 'btlist',
        'InfoText': 704,   #704 #msgid "Configure and pair Bluetooth devices"
        }}
    ENABLED = False
    OBEX_ROOT = None
    OBEX_DAEMON = None
    ...
    def discover_devices(self):
        ...
        if not dbus_bluez.system_has_bluez():
            self.found_devices = frozenset()
            oe.winOeMain.getControl(1301).setLabel(oe._(32346))   #32346 #msgid "Bluetooth is disabled"
        ...
        if self.dbusBluezAdapter is None:
            self.found_devices = frozenset()
            oe.winOeMain.getControl(1301).setLabel(oe._(32338))   #32338 #msgid "No Bluetooth adapter found."
            oe.winOeMain.getControl(int(oe.listObject['btlist'])).reset()
            self.clear_list()
        ...
        if self.dbusDevices:
            ...
            self.found_devices = frozenset()
            oe.winOeMain.getControl(1301).setLabel(oe._(32339))   #32339 #msgid "No Bluetooth device found. Please put your Bluetooth device into discovery mode[CR]and start the scan"
            oe.winOeMain.setProperty('show_bt_label', 'true')
        for dbusDevice, device_properties in self.dbusDevices.items():
            ...
            for prop in self.properties:
                ...
                if name in device_properties:
                    ...
                    if name == 'Connected':
                        if value:
                            dictProperties['ConnectedState'] = oe._(32334)   #32334 #msgid "Yes"
                        else:
                            dictProperties['ConnectedState'] = oe._(32335)   #32335 #msgid "No"
    ...
    def open_context_menu(self, listItem):
        ...
        if listItem.getProperty('Paired') != '1':
            values[1] = {
                'text': oe._(32145),   #32145 #msgid "Pair"
                'action': 'init_device',
                }
            if listItem.getProperty('Trusted') != '1':
                values[2] = {
                    'text': oe._(32358),   #32358 #msgid "Trust and Connect"
        ...
        if listItem.getProperty('Connected') == '1':
            ...
            if listItem.getProperty('entry') in devices:
                values[4] = {
                    'text': oe._(32389),   #32389 #msgid "Disable Standby"
                    'action': 'disable_device_standby',
                    }
            else:
                values[4] = {
                    'text': oe._(32388),   #32388 #msgid "Enable Standby"
    ...
    @log.log_function()
    def open_pinkey_window(self, runtime=60, title=32343):   #32343 #msgid "Enter the following Key on your Device."
        self.pinkey_window = oeWindows.pinkeyWindow('service-FunHomeTV-Settings-getPasskey.xml', oe.__cwd__, 'Default')
