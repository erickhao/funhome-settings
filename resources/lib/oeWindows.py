...
# Copyright (C) 2019-present FunHomeTV (https://funhome.tv)
        ...
        try:
            ...
            if controlID in self.guiLists:
                ...
                if strTyp == 'multivalue':
                    ...
                    xbmcDialog = xbmcgui.Dialog()
                    returnValue = xbmcDialog.browse(1, 'FunHome.TV', 'files', '', False, False, '/')
                    if returnValue != '' and returnValue != '/':
                        selectedItem.setProperty('value', str(returnValue))
                elif strTyp == 'folder':
                    xbmcDialog = xbmcgui.Dialog()
                    returnValue = xbmcDialog.browse(0, 'FunHome.TV', 'files', '', False, False, '/storage')
                    if returnValue != '' and returnValue != '/':
                        selectedItem.setProperty('value', str(returnValue))
                elif strTyp == 'ip':
                    if strValue == '':
                        strValue = '0.0.0.0'
                    xbmcDialog = xbmcgui.Dialog()
                    returnValue = xbmcDialog.numeric(3, 'FunHome.TV', strValue)
                    if returnValue != '':
                    ...
                    if strValue == 'None' or strValue == '':
                        strValue = '0'
                    xbmcDialog = xbmcgui.Dialog()
                    returnValue = xbmcDialog.numeric(0, 'FunHome.TV', strValue)
                    if returnValue != '':
                        selectedItem.setProperty('value', returnValue)
                elif strTyp == 'bool':
                    ...
                    if strValue == '0':
                        ...
                        selectedItem.setProperty('value', '1')
                elif strTyp == 'textTriggerButton':
                    if hasattr(oe.dictModules[selectedMenuItem.getProperty('modul')], selectedItem.getProperty('action')):
                        getattr(oe.dictModules[selectedMenuItem.getProperty('modul')], selectedItem.getProperty('action'))(listItem=selectedItem)
                if (selectedItem.getProperty('action') != '') and (strTyp != 'textTriggerButton'):
                    if hasattr(oe.dictModules[selectedMenuItem.getProperty('modul')], selectedItem.getProperty('action')):
                        getattr(oe.dictModules[selectedMenuItem.getProperty('modul')], selectedItem.getProperty('action'))(listItem=selectedItem)
...
class pinkeyWindow(xbmcgui.WindowXMLDialog):
    ...
    def set_title(self, text):
        self.getControl(1700).setLabel(text)   #700 #msgid "Configure a hostname, keyboard type, perform a reset, update, or backups and restores"

    def set_label1(self, text):
        self.getControl(1701).setLabel(str(text))   #701 #msgid "Configure network startup options, NTP and VPNs"

    def set_label2(self, text):
        self.getControl(1702).setLabel(str(text))   #702 #msgid "Manage the Ethernet, Wireless and VPN connections available to the system"

    def set_label3(self, text):
        self.getControl(1703).setLabel(str(text))   #703 #msgid "Configure system services like Samba, SSH, Cron, Avahi and Syslog"

    def append_label3(self, text):
        label = self.getControl(1703).getLabel()   #703 #msgid "Configure system services like Samba, SSH, Cron, Avahi and Syslog"
        self.getControl(1703).setLabel(label + str(text))   #703 #msgid "Configure system services like Samba, SSH, Cron, Avahi and Syslog"

    def get_label3_len(self):
        return len(self.getControl(1703).getLabel())   #703 #msgid "Configure system services like Samba, SSH, Cron, Avahi and Syslog"
...
class wizard(xbmcgui.WindowXMLDialog):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.wizWinTitle = 32300   #32300 #msgid "Welcome to @DISTRONAME@"
        self.guisettings = f'{oe.XBMC_USER_HOME}/userdata/guisettings.xml'
        oe.dbg_log('haoyq oeWindows-wizard::__init__' , 'enter_function', 0)
        self.buttons = {
            1: {
                'id': 1500, #next
                'modul': '',
        ...
        oe.dbg_log('haoyq oeWindows-wizard::__init__' , 'exit_function', 0)

    def onInit(self):
        self.visible = True
        try:
            oe.dbg_log('haoyq oeWindows-wizard::onInit' , 'enter_function', 0)
            self.setProperty('arch', oe.ARCHITECTURE)
            self.setProperty('distri', oe.DISTRIBUTION)
            self.setProperty('version', oe.VERSION)
            self.setProperty('build', oe.BUILD)
            ...
            self.getControl(self.radiobuttons[3]['id']).setVisible(False)
            self.getControl(self.buttons[2]['id']).setVisible(False)
            if oe.BOOT_STATUS == "SAFE":
              self.set_wizard_title(f"[COLOR red][B]{oe._(32393)}[/B][/COLOR]")  #32393 #msgid "** SAFE MODE! ** SAFE MODE! ** SAFE MODE! **"
              self.set_wizard_text(oe._(32394))    #32394 #msgid "@DISTRONAME@ has temporarily started in safe mode due to repeated Kodi crashes.[CR][CR]You may now investigate the cause of the crashes by enabling ssh or Samba.[CR][CR]Your original Kodi installation can be accessed via "/storage/.kodi.FAILED" and the Samba "Kodi-Failed" share.[CR][CR]Rebooting will return to your original Kodi installation.[CR][CR]Go to https://forum.funhometv.tv if you require further assistance. When posting to the forum include the link to your crash log which can be viewed by clicking the crash log option in Settings > FunHomeTV > System > Submit Log."ok= change funhometv.tv --> funhome.tv
            else:
                if oe.FACTORY_STATUS == 'FAILED':
                    self.set_wizard_title("[COLOR red][B]%s[/B][/COLOR]" % oe._(32546).encode('utf-8'))   #32546 #msgid "** NETWORK FAILED MODE! **"
                    self.set_wizard_text(oe._(32547))   #32547 #msgid "@DISTRONAME@ has temporarily started in factory failed mode due to Network problem.-- Is network line connected with AP ?
                else:
                    self.set_wizard_title(oe._(32301))   #32301 #msgid "Welcome"
                    self.set_wizard_text(oe._(32302))     #32302 #msgid "This wizard will guide you through the process of setting up your new @DISTRONAME@ installation - setting your location, timezone and connecting you to the internet.[CR][CR]These settings can be changed later by navigating to Programs > @DISTRONAME@ Settings."=== add description of input zerotier network id  if both chinese and english
                    oe.winOeMain.set_wizard_button_title(oe._(32310))  #32310 #msgid "Language:"
                cur_lang = xbmc.getLanguage()
                oe.winOeMain.set_wizard_button_1(cur_lang, self, 'wizard_set_language')
            self.showButton(1, 32303)    #32303 #msgid "Next"
            self.setFocusId(self.buttons[1]['id'])
            oe.showlicense()
            oe.dbg_log('haoyq:oeWindows-wizard::onInit' , 'exit_function', 0)
        except Exception as e:
    ...
    def wizard_set_language(self):
        ...
        try:
            ...
            cur_lang = xbmc.getLanguage()
            langIndex = 0
            oe.dbg_log('oeWindows::wizard_set_language-cur_lang from xbmc.getLanguage', repr(cur_lang), 0)
            for index, lang in enumerate(languagesList):
                if cur_lang in lang:
                    ...
                    pass
            selLanguage = xbmcDialog.select(oe._(32310), languagesList, preselect=langIndex)     #32310 #msgid "Language:"
            if selLanguage >= 0:
                langKey = languagesList[selLanguage]
                lang_new = langCodes[langKey]
                oe.dbg_log('oeWindows::wizard_set_language:lang_new:',str(lang_new), 0)
    ...
    def set_wizard_radiobutton_1(self, label, modul, action, selected=False):
        try:
            ...
            self.getControl(self.radiobuttons[1]['id']).controlLeft(self.getControl(self.radiobuttons[3]['id']))
            ...
            self.getControl(self.buttons[1]['id']).controlUp(self.getControl(self.radiobuttons[3]['id']))
            self.getControl(self.buttons[1]['id']).controlLeft(self.getControl(self.buttons[2]['id']))
    ...
    def set_wizard_radiobutton_2(self, label, modul, action, selected=False):
        try:
            ...
            self.getControl(self.radiobuttons[2]['id']).controlRight(self.getControl(self.radiobuttons[3]['id']))
            self.getControl(self.buttons[1]['id']).controlUp(self.getControl(self.radiobuttons[3]['id']))
            self.getControl(self.buttons[1]['id']).controlLeft(self.getControl(self.buttons[2]['id']))
            self.getControl(self.buttons[2]['id']).controlUp(self.getControl(self.radiobuttons[1]['id']))
            self.getControl(self.buttons[2]['id']).controlRight(self.getControl(self.buttons[1]['id']))
            self.getControl(self.radiobuttons[1]['id']).controlRight(self.getControl(self.radiobuttons[2]['id']))
            self.getControl(self.radiobuttons[3]['id']).controlLeft(self.getControl(self.radiobuttons[2]['id']))
            self.getControl(self.radiobuttons[2]['id']).setSelected(selected)
        except Exception as e:
            oe.dbg_log('oeWindows.wizard::set_wizard_radiobutton_2()', f'ERROR: ({repr(e)})')

    def set_wizard_radiobutton_3(self, label, modul, action, selected=False):
        try:
            self.radiobuttons[3]['modul'] = modul
            self.radiobuttons[3]['action'] = action
            self.getControl(self.radiobuttons[3]['id']).setLabel(label)
            self.getControl(self.radiobuttons[3]['id']).setVisible(True)
            self.getControl(self.radiobuttons[3]['id']).controlLeft(self.getControl(self.radiobuttons[2]['id']))
            self.getControl(self.radiobuttons[3]['id']).controlDown(self.getControl(self.buttons[1]['id']))
            self.getControl(self.radiobuttons[3]['id']).controlRight(self.getControl(self.radiobuttons[1]['id']))
            self.getControl(self.buttons[1]['id']).controlUp(self.getControl(self.radiobuttons[3]['id']))
            self.getControl(self.buttons[1]['id']).controlLeft(self.getControl(self.buttons[2]['id']))
            self.getControl(self.buttons[2]['id']).controlUp(self.getControl(self.radiobuttons[1]['id']))
            self.getControl(self.buttons[2]['id']).controlRight(self.getControl(self.buttons[1]['id']))
            self.getControl(self.radiobuttons[1]['id']).controlRight(self.getControl(self.radiobuttons[2]['id']))
            self.getControl(self.radiobuttons[2]['id']).controlLeft(self.getControl(self.radiobuttons[1]['id']))
            self.getControl(self.radiobuttons[3]['id']).setSelected(selected)
        except Exception as e:
            oe.dbg_log('oeWindows.wizard::set_wizard_radiobutton_3()', f'ERROR: ({ repr(e)})')
    ...
    def onClick(self, controlID):
        ...
        global prevModule
        try:
            oe.dbg_log(f'wizard::onClick({str(controlID)})', 'enter_function', 0)
            for btn in self.buttons:
            ...
            if controlID == 1501:
                ...
                if strModule == "connman":# haoyq should be the second wizard ,so do init  #original is "system"
                    #self.getControl(1390).setLabel('1') # added for "next then previous" not hide
                    oe.xbmcm.waitForAbort(0.5)
                    self.getControl(1391).setLabel('') # hide less than 1390 , except buttons.
                    #self.getControl(self.buttons[3]['id']).setVisible(False)
                    #self.getControl(self.buttons[4]['id']).setVisible(False)
                    #self.getControl(self.radiobuttons[1]['id']).setVisible(False)
                    #self.getControl(self.radiobuttons[2]['id']).setVisible(False)
                    ...
                    self.onClick(1500)
                #dupped self.oe.dbg_log('wizard::onClick(' + unicode(controlID) + ')', 'exit_function', 0)

            if controlID == 1500:
                self.getControl(1390).setLabel('1') # a point hide all control except Next and previous
                oe.xbmcm.waitForAbort(0.5)
                ...
                self.getControl(self.radiobuttons[2]['id']).setVisible(False)
                self.getControl(self.radiobuttons[3]['id']).setVisible(False)
                self.showButton(2, 32307)   #32307 #msgid "Previous"
                self.set_wizard_title('')
                self.set_wizard_text('')
                self.set_wizard_list_title('')
                self.set_wizard_button_title('')
                ...
                for module in sorted(oe.dictModules, key=lambda x: list(oe.dictModules[x].menu.keys())):
                    strModule = module
                    oe.dbg_log('wizard::onClick(' + str(controlID) + ')', 'in for loop of module:' + repr(module) + 'menu:' + repr(oe.dictModules[module].menu), 0)
                    if hasattr(oe.dictModules[strModule], 'do_wizard') and oe.dictModules[strModule].ENABLED:
                        ...
                        if self.wizards != []:
                            ...
                            oe.dbg_log('wizard::onClick(' + str(controlID) + ')', 'in for loop of module:' + repr(module) + 'wizards not empty,wizards:' + repr(self.wizards) + ',PrevModule:' + repr(prevModule), 0)
                        if oe.read_setting(strModule, 'wizard_completed') == None and strModule not in self.wizards:
                            self.last_wizard = strModule
                            oe.dbg_log('wizard::onClick(' + str(controlID) + ')', 'last_wizard(or strModule):' + repr(strModule)+'and calling it later', 0)
                            if hasattr(oe.dictModules[strModule], 'do_init'):
                                oe.dictModules[strModule].do_init()
                            self.getControl(1390).setLabel('') #display buttons
                            oe.dictModules[strModule].do_wizard()
                            self.wizards.append(strModule)
                            oe.dbg_log('wizard::onClick(' + str(controlID) + ')', 'in for loop of module:' + repr(module) + 'after append(strModule),wizards:' + repr(self.wizards) + ',PrevModule:' + repr(prevModule), 0)
                            oe.write_setting(strModule, 'wizard_completed', 'True')
                            oe.dbg_log('wizard::onClick(' + str(controlID) + ')', 'setting strModule:' + repr(strModule) + 'wizard_completed', 0)
                ...
                if self.is_last_wizard == True:
                    ...
                    oe.dbg_log(f'wizard::onClick-continue=xbmc.executebuiltin=',repr(lang_new), oe.LOGDEBUG)
                    oe.xbmcm.waitForAbort(0.5)
                    xbmc.executebuiltin(f'SendClick(10100,11)')
                    oe.dbg_log(f'wizard::onClick-continue=xbmc.executebuiltin=','SendClick(10100,11)', oe.LOGDEBUG)
                    oe.write_setting('funhometv', 'wizard_completed', 'True')
                    oe.dbg_log(f'wizard::onClick-continue=write-settings=','wizard_completed', oe.LOGDEBUG)
                    self.visible = False
                    self.close()
                    if lang_new:
                        #for _ in range(20):
                ...
                oe.dbg_log('wizard::onClick(' + str(controlID) + ')', 'is_last_wizard=' + repr(self.is_last_wizard), 0)
            oe.dbg_log('wizard::onClick(' + str(controlID) + ')', 'exit_function', 0)
        except Exception as e:
